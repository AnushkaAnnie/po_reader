...
def _is_parseable_date(s: str) -> bool:
    formats = [
        ...
        "%d %b %Y", "%d %B %Y", "%Y/%m/%d",
        "%d/%m/%y", "%m/%d/%Y",
        # Partial formats (no year) — e.g. "27 Sept", "10 Sept"
        "%d %b", "%d %B",
    ]
    for fmt in formats:
        try:
            ...
            continue
    # Accept portal-style strings like "10 Sept+49d", "10 Sept +49d", "27 Sept +0d"
    if re.match(r"^\d{1,2}\s+[A-Za-z]+(\s*\+\d+d)?$", s.strip()):
        return True
